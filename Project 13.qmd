...
library(Boruta)
set.seed(123)
boruta_output <- Boruta(PovertyStatus ~ ., data = ds_tr, doTrace = 2)
print(boruta_output)
...
# Downsampling the majority class for balance
set.seed(123)  # For reproducibility
...
index <- sample(1:nrow(balanced_data))
n_tr <- round(0.70 * nrow(balanced_data))
n_val <- round(0.15 * nrow(balanced_data))

ds_tr <- balanced_data[index[1:n_tr], ]  # First 70% for training
ds_val <- balanced_data[index[(n_tr + 1):(n_tr + n_val)], ]  # Next 15% for validation
ds_te <- balanced_data[index[(n_tr + n_val + 1):nrow(prepared_dataset)], ] #Remaining for testing
