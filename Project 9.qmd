...
```{r} #| code-fold: true #| code-summary:}
"show" library(Boruta) set.seed(123) boruta_output <- Boruta(PovertyStatus ~ ., data = subset_data, doTrace = 2) print(boruta_output) plot(boruta_output, las = 2, cex.axis = 0.7, xlab = "", main = "Variable Importance")
```
...
Finally, after the result of the confussion mattrix and metrics the results of the model and the accuracy has increased really little amount, but at least it has increased. As the model Support Vector Machine is not to much interpretable, we can just use the model if after comparing with the other models it has a better accuracy and balanced accuracy.
