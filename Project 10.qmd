...
### 3.3 Dimension Reduction & Variable Importance
...
```{r}
#| code-fold: true #| code-summary: "show" 
set.seed(145) subset_data <- prepared_dataset %>%   sample_n(10000) print(dim(subset_data)) subset_data <- subset_data[-c(1,8,10)]
```
...
`{r} #| code-fold: true #| code-summary:} "show" library(Boruta) set.seed(123) boruta_output <- Boruta(PovertyStatus ~ ., data = subset_data, doTrace = 2) print(boruta_output) plot(boruta_output, las = 2, cex.axis = 0.7, xlab = "", main = "Variable Importance")`
...
First a Linear support vector Machine model was created, and then a prediction was performed with the model using the validation set, in order to evaluate it with the confussion Matrix and obtain the model evaluation metrics. But up to this point it is observed that there is not a big gap between the Accucay and the Balanced accuracy, thanks to the previous downsampling performed.
...
Then the model was trained with Cross Validation to confirm that the model is able to generalize new data well. For now, the validation accuracy is high (≈74%). This is normal since this accuracy is computed on the training set.
...
At this point, a support vector Machine model was created with a radial kernel, and then a prediction was performed with the model using the validation set, in order to evaluate it with the confussion Matrix and obtain the model evaluation metrics. It is observed that both the Accuracy and the Balanced Accuracy of this new model are better than the linear SVM model, although only by a few points. It is necessary to look for Tuning the SVM radial.
...
Then the model was re-trained with the entire training set using optimal hyperparameters for the radial basis kernel. Finally, after the result of the confussion matrix of the tunned model with a new Gamma and C , the accuracy has increased a really little amount , but at least it has increased. In othe side, as the model Support Vector Machine is not to much interpretable, we can just use the model if after comparing with the other models it has a better accuracy and balanced accuracy.
