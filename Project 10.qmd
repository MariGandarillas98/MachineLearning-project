...
### 4.1.2 Dimension Reduction & Variable Importance
...
set.seed(145) 
subset_data <- prepared_dataset %>%   sample_n(10000) print(dim(subset_data)) subset_data <- subset_data[-c(1,8,10)]
```
...
Finally, after the result of the confussion mattrix and metrics the results of the model and the accuracy has increased in small proportions, but at least it has increased. As the model Support Vector Machine is not interpretable, we can just use the model if after comparing with the other models it has a better accuracy and balanced accuracy.
