...
```{r}
#REducing data base to work with clustering methods
cl_tr <- prepared_dataset[index[1:n_tr], ]  # First 70% for training

cl_tr<-cl_tr[-c(1,10)]

#downsampling
set.seed(123) ## for reproducibility
cl_tr_down <- downSample(subset(cl_tr, select=-PovertyStatus), y=ds_tr$PovertyStatus, yname="PovertyStatus")

# Preparing data
cl_tr_down <- cl_tr_down %>%
  dplyr::select(-PovertyStatus) %>%
  scale()

```

```{r}
# Visualization of the evaluation of the optimal number of clusters using the silhouette method.
fviz_nbclust(cl_tr_down, hcut, method = "silhouette", hc_metric = "manhattan", 
             hc_method = "complete", k.max = 25, verbose = FALSE)

fviz_nbclust(cl_tr_down,
...
dist_mat <- dist(cl_tr_down, method = "manhattan")
...
#  Ensure that clusters is a vector of integers
clusters <- as.integer(clusters)  

#  Calculate the silhouette measures for the assigned clusters

silhouette_values <- silhouette(clusters, dist_mat)

# Visualizalization Silhouette Plot
silhouette_plot <- fviz_silhouette(silhouette_values)
...
data_clust <- cutree(hc_complete, k=2)
data_clust
```

```{r}
data_comp <- data.frame(cl_tr_down, Clust=factor(data_clust), Id=row.names(cl_tr_down))
...
pov_pca <- PCA(cl_tr_down[,-12], ncp = 11, graph = FALSE)
pov_pca
```

```{r}
fviz_eig(pov_pca, addlabels = TRUE, ncp=11)
fviz_contrib(pov_pca, choice = "var", axes = 2)
fviz_contrib(pov_pca, choice = "var", axes = 1)
```

```{r}
pov_hc <- hclust(dist(cl_tr_down[,-12], method = "manhattan"))
#sine we want to explain more than 70% we choose k=5
pov_clust <- cutree(pov_hc, k = 5)
fviz_pca_biplot(pov_pca,
             col.ind = factor(pov_clust))
```
