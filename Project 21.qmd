...
ggplot(prepared_dataset, aes(x = Log_Age)) +
  geom_histogram(binwidth = 0.1, fill = "#5E81ACFF", color = "black", alpha = 0.7) +
...
The first step was to prepare the data, due to the memory capacity it was decided to work with the data of the Validation Set, and those variables that are nominal were transformed into factors. Then according to the Silhouette method it was determined to have 2 clusters, which were used for the following steps. It was continued with Silhouette plot and it was observed that the second cluster is not as homogeneous as the first one. And then the dendogram and the division of the two clusters were visualized.
...
\# Plot para visualizar la silueta promedio en función del número de clusters
...
#| warning: false
#| message: false
# Calculating the distance matrix using the Manhattan metric
...
plot(ds_hc, hang=-1)
rect.hclust(ds_hc, k=2)
data_clust <- cutree(ds_hc, k=2)
data_clust
```

Then we proceeded to observe boxplots of each variable and we observed that the second cluster gathers more people considered poor, and this group gathers more women than men, which could be related to the difference in income that usually affects women negatively; also the second cluster gathers more people who studied in high school, which is associated to the fact that the most important variable in the Tree Method is the education variable, and the people with bachelor's degree are in greater proportion in the first cluster. On the other hand, the second cluster has a higher proportion of people of white race, which could be related in some way to the background of racism exposed in the background of this work. Continuing in the second cluster, a greater proportion of people living in the southern part of the United States are in the second cluster.
